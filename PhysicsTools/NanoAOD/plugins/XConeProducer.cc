...
class XConeProducer : public edm::stream::EDProducer<> {
  ...
  const std::string topjetCollName_;
  ...
  doRekey_(iConfig.exists("doRekey") ? iConfig.getParameter<bool>("doRekey") : false)
{
  ...
  produces<pat::JetCollection>(topjetCollName_);
...
void XConeProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (_psj.size() < minParticleCount) {
    ...
    auto topjetCollection = std::make_unique<pat::JetCollection>();
    iEvent.put(std::move(topjetCollection), topjetCollName_);
  ...
  for(unsigned int i=0; i<fatjets.size(); i++){
    ...
    if (enoughParticles && doSubjets) {
      ...
      for (unsigned int j = 0; j < subjets.size(); ++j) {
        ...
        for (const auto& particle : subjets[j].constituents()) {
        ...
        // cout<<" - Nsubjettiness - subjets "<<endl;
        double beta = 0.1;
        ...
        Nsubjettiness nSub4(4, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 0.4));
        // cout<<"\t - set "<<endl;
        double tau1 = nSub1(subjets[j]);
        double tau2 = nSub2(subjets[j]);
        double tau3 = nSub3(subjets[j]);
        double tau4 = nSub4(subjets[j]);
    ...
    // Nsubjettiness
    // cout<<" - Nsubjettiness - fatjets "<<endl;
    double beta = 1.0;
    ...
    Nsubjettiness nSub4(4, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 1.2));
    // cout<<"\t - set "<<endl;
    double tau1 = nSub1(fatjets[i]);
    double tau2 = nSub2(fatjets[i]);
    double tau3 = nSub3(fatjets[i]);
    double tau4 = nSub4(fatjets[i]);
    ...
    patJet.addUserFloat("tau4", tau4);
    jetCollection->push_back(patJet);

    // /////////////////////////////////////////////
    // /// Topjets
    // /// Cluster again one jet from all particles,
    // /// that are clustered from subjets

    // cout<<" - create pseudo jet - topjets "<<endl;
    std::unique_ptr<NjettinessPlugin> plugin_xcone_top;
    initPlugin(plugin_xcone_top, 1, RJets_, BetaJets_, usePseudoXCone_);
    double ghost_maxrap = 4.0;      // maxiumum y of ghost particles

    JetDefinition jet_def_top(plugin_xcone_top.get());
    ClusterSequenceArea clust_seq_top(particles, jet_def_top, area_def);
    vector<PseudoJet> topjets = sorted_by_pt(clust_seq_top.inclusive_jets(0));

    if(topjets.size()!=1){
      edm::LogWarning("XConeTooFewTopJets") << "Found " << topjets.size() << " jets but only requested one jet.\n"
                                         << "Added at least one jet." << endl;
      // Check we got the number of subjets we asked for
      for (uint iJet=topjets.size(); iJet < NJets_; iJet++) {
        topjets.push_back(PseudoJet(0, 0, 0, 0));
      }                                   
    }

    PseudoJet topjet = topjets[0];

    // get SoftDrop Mass for every fat jet
    PseudoJet sdjet = sd(topjet);
    float sd_topmass = sdjet.m();

    // jet area
    double topjet_area = 0;
    if(topjet.has_area())
      topjet_area = topjet.area();

    // Nsubjettiness
    // cout<<" - Nsubjettiness - topjets "<<endl;
    Nsubjettiness nSub1_top(1, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 1.2));
    Nsubjettiness nSub2_top(2, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 1.2));
    Nsubjettiness nSub3_top(3, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 1.2));
    Nsubjettiness nSub4_top(4, OnePass_WTA_KT_Axes(), NormalizedMeasure(beta, 1.2));
    // cout<<"\t - set "<<endl;
    double tau1_top = nSub1_top(topjet);
    double tau2_top = nSub2_top(topjet);
    double tau3_top = nSub3_top(topjet);
    double tau4_top = nSub4_top(topjet);

    // cout<<"\t - add "<<endl;
    // cout<<"\t - create pat jet - topjets "<<endl;
    pat::Jet patTopjet = createPatJet(topjet, iSetup);
    if (doRekey_) {
      patTopjet = rekeyJet(patTopjet, rekeyCandHandle);
    }
    // cout<<"\t - add variable to topjet"<<endl;
    patTopjet.setJetArea(topjet_area);
    patTopjet.addUserFloat("softdropmass", sd_topmass);
    patTopjet.addUserFloat("tau1", tau1_top);
    patTopjet.addUserFloat("tau2", tau2_top);
    patTopjet.addUserFloat("tau3", tau3_top);
    patTopjet.addUserFloat("tau4", tau4_top);
    topjetCollection->push_back(patTopjet);
...
pat::Jet XConeProducer::createPatJet(const PseudoJet & fjJet, const edm::EventSetup& iSetup)
{
  ...
  if(fjJet.px()==0 && fjJet.py()==0 && fjJet.pz()==0){//jet or sub-jet was created artificially
    ...
    reco::PFJet pfjet;
    reco::writeSpecific(pfjet,jet4v,vertex_,constituents, iSetup);// https://github.com/cms-sw/cmssw/blob/012956d4c9c1500ac605b5b070fcd6b808341714/RecoJets/JetProducers/src/JetSpecific.cc#L68
