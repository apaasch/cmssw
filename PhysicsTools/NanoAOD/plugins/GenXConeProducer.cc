...
GenXConeProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(unsigned int i=0; i<fatjets.size(); i++){
    ...
    std::vector<PseudoJet> particles_from_subjets;
    for (uint s=0; s<subjets.size(); s++) {
      ...
      double tau1 = nSub1(subjets[s]);
      double tau2 = nSub2(subjets[s]);
      double tau3 = nSub3(subjets[s]);
      double tau4 = nSub4(subjets[s]);
